...
if ($template instanceof Template)
{
	if (!empty($_GET['pid']))
	{
		...
		for($i = 0; $i < count($keys); $i += 2)
		{
			...
			if(empty($keys[$i + 1]))
				$template->assignFromNew('projectHeadContent', 'box/editProjectHeadRow.xhtml', array('key1' => $key1, 'value1' => $value1, 'key2' => '', 'value2' => ''));
			else
				$template->assignFromNew('projectHeadContent', 'box/editProjectHeadRow.xhtml', array('key1' => $key1, 'value1' => $value1, 'key2' => $keys[$i + 1], 'value2' => $project->info[$keys[$i + 1]]));
		}

		for($i = count($keys)/2; $i <= 3; $i ++)
			$template->assignFromNew('projectHeadContent', 'box/editProjectHeadRow.xhtml', array('key1' => '', 'value1' => '', 'key2' => '', 'value2' => ''));
		
		foreach($project->versions as $value)
			$template->assignFromNew('versions', 'box/option.xhtml', array('value' => $value));
		
		$template->assign('version', $project->version);
