...
if ($template instanceof Template)
{
	if (!empty($_GET['pid']))
	{
		...
		for($i = 0; $i < count($keys); $i += 2)
		{
			...
			if(empty($keys[$i + 1]))
				$projectTemplate->assignFromNew('projectHeadContent', 'box/projectHeadRow.xhtml', array('key1' => $key1, 'value1' => $value1, 'key2' => '', 'value2' => ''));
			else
				$projectTemplate->assignFromNew('projectHeadContent', 'box/projectHeadRow.xhtml', array('key1' => $key1, 'value1' => $value1, 'key2' => $keys[$i + 1], 'value2' => $project->info[$keys[$i + 1]]));
		}
